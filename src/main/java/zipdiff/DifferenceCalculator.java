...
public class DifferenceCalculator {
    ...
    private boolean compareCompressedSizes = false;
    private Pattern filesToIgnorePattern;
    private boolean bVerbose = false;

    private HashMap<ZipEntry, HashCode> hashCodeMap = new HashMap<>();
    ...
    public boolean getCompareCompressedSizes() {
        return compareCompressedSizes;
    }

    public void setCompareCompressedSizes(boolean compareCompressedSizes) {
        this.compareCompressedSizes = compareCompressedSizes;
    }
    ...
    public void setFilenameRegexToIgnore(Set<String> patterns) {
        if (patterns == null) {
            ...
            String regex = patterns.stream().map(pattern -> "(" + pattern + ")")
    ...
    protected boolean isCVSFile(String filepath, String filename)
    {
    	...
    	else if ( (filepath.contains("CVS")) || (filename.equals("CVS")))
		{
			return true;
		}
		else
    ...
    protected void processZipEntry(String prefix, ZipEntry zipEntry, InputStream is, Map<String, ZipEntry> zipEntryMap) throws IOException {
        if (ignoreThisFile(prefix, zipEntry.getName())) {
            ...
            if (zipEntry.isDirectory()) {
                ...
                zipEntryMap.put(name, zipEntry);
                hashCodeMap.put(zipEntry, hashFunction.hashBytes(ByteStreams.toByteArray(is)));
            }
        }
    }
    ...
    protected boolean entriesMatch(ZipEntry entry1, ZipEntry entry2) {
        ...
        if (getCompareCompressedSizes()) {
            result = result && (entry1.getCompressedSize() == entry2.getCompressedSize());
        }
        ...
        if (result && !entry1.isDirectory()) {
            System.out.println(entry1.getName() + " " + hashCodeMap.get(entry1));
            System.out.println(entry2.getName() + " " + hashCodeMap.get(entry2));
            result = hashCodeMap.get(entry1).equals(hashCodeMap.get(entry2));
        }
    ...
    public static void main(String[] args) {
        HashFunction hashFunction = Hashing.goodFastHash(10);
        HashCode hashCode = hashFunction.hashBytes(new byte[] {0,1,2});
    }
}
